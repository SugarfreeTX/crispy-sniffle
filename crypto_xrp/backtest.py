...
# present in Yahoo Finance daily data. No market-open gate is needed here.
...
try:
    from crypto_xrp.backtest_strategy import XRPDailyBacktestStrategy
except ModuleNotFoundError:
    from backtest_strategy import XRPDailyBacktestStrategy

DEFAULT_DATA_PATH = BASE_DIR / "data" / "xrp_daily.csv"
DEFAULT_OUTPUT_DIR = BASE_DIR / "backtest_outputs"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run deterministic XRP backtest using backtesting.py")
    parser.add_argument("--symbol", default="XRP-USD", help="Ticker symbol for data refresh mode")
    ...
    parser.add_argument("--min-atr", type=float, default=0.005, help="Minimum ATR required to trade (XRP fractional-dollar scale; MSFT default 3.5 would block all XRP trades)")
    parser.add_argument("--max-atr", type=float, default=1.50, help="Maximum ATR allowed to trade (XRP: $1.50 filters extreme flash-crash days)")
    ...
    # Sweep ranges recalibrated for crypto: wider RSI bands, XRP-scale ATR values
    parser.add_argument("--sweep-buy-pullback-rsi", default="42,47,52",
                        help="Comma-separated BUY pullback RSI values (crypto: slightly higher ceiling than MSFT)")
    parser.add_argument("--sweep-sell-overbought-rsi", default="75,80,85",
                        help="Comma-separated SELL overbought RSI values (crypto: lower ceiling; XRP can reverse faster than MSFT at extreme RSI)")
    parser.add_argument("--sweep-min-atr", default="0.001,0.003,0.005,0.01",
                        help="Comma-separated minimum ATR values (XRP fractional-dollar scale; MSFT default 0.5-3.5 would block all XRP trades)")
    parser.add_argument("--sweep-extreme-setup-rsi", default="20,25,30",
                        help="Comma-separated extreme setup RSI values (crypto: lower floor; XRP oversold can reach <20)")
    parser.add_argument("--sweep-extreme-setup-rel-vol", default="0.8,1.0,1.2",
                        help="Comma-separated extreme setup relative-volume values (slightly lower threshold for crypto thinner books)")
...
def run_parameter_sweep(
    ...
    sweep_results_path = output_dir / f"sweep_results_{args.symbol.lower().replace('-', '_')}_{run_id}.csv"
    sweep_top_path = output_dir / f"sweep_top_{args.symbol.lower().replace('-', '_')}_{run_id}.csv"
    best_config_path = output_dir / f"sweep_best_{args.symbol.lower().replace('-', '_')}_{run_id}.json"
...
def write_outputs(
    ...
    if isinstance(trades, pd.DataFrame) and not trades.empty:
        trades.to_csv(output_dir / f"trades_{safe_symbol}_{run_id}.csv", index=False)

    equity_curve = stats.get("_equity_curve")
    if isinstance(equity_curve, pd.DataFrame) and not equity_curve.empty:
        equity_curve.to_csv(output_dir / f"equity_{safe_symbol}_{run_id}.csv")
    ...
    with open(output_dir / f"metrics_{safe_symbol}_{run_id}.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    with open(output_dir / f"stats_{safe_symbol}_{run_id}.txt", "w", encoding="utf-8") as f:
        f.write(str(stats))
