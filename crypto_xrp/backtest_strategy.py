...
# NOTE: No pandas_market_calendars. XRP-USD trades 24/7; all calendar days are
# present in Yahoo Finance daily bars. No market-open gate is needed.
...
def should_auto_hold(
    ...
    """Replicate the local HOLD gate from the live XRP loop for deterministic backtests.

    Args:
        packet: Full market+portfolio packet dict.
        thresholds: Optional override dict for RSI/volume threshold parameters.
...
# ── XRP crypto adaptation notes ───────────────────────────────────────────────
# Adapted from equity_msft/backtest_strategy.py. Key differences vs MSFT:
#   1. No market calendar check. XRP-USD Yahoo Finance data covers all 7 calendar
#      days as daily bars; backtesting.py iterates whatever rows are present.
#   2. portfolio["shares"] is integer XRP token count, not equity shares. The
#      _current_shares() method already returns int(round(...)) — correct as-is.
#   3. ATR defaults recalibrated for XRP's fractional-dollar price scale:
#      min_atr=0.005, max_atr=1.50 (matching live crypto_xrp/complete_daily_loop.py).
#      MSFT defaults (min 3.5, max 18.0) would block every XRP trade.
#   4. Position sizing via risk_per_trade_pct still works at XRP's $2-3 price:
#      risk_amount / (atr_14 * 2) naturally produces token quantities in the
#      hundreds-to-thousands range and is capped by max_position_size_pct.
#   5. Partial sell thresholds widened: crypto 3-8% daily swings are normal noise.
#      The MSFT 8% full-exit floor would trigger almost every day; 15% is used here.
# ─────────────────────────────────────────────────────────────────────────────
class XRPDailyBacktestStrategy(Strategy):
    """Deterministic daily-bar strategy adapted from the live XRP loop."""

    risk_per_trade_pct = 0.02
    max_position_size_pct = 0.20
    max_drawdown_pct = 0.10
    min_atr = 0.005   # XRP-appropriate: $0.005 minimum daily ATR (fractional-cent scale)
    max_atr = 1.50    # XRP-appropriate: $1.50 maximum ATR (extreme flash-move filter)
    slippage_bps = 5.0
    max_consecutive_losses = 999  # disabled for backtest; CLI sweep default stays 5
    regime_caution_days = 18
    history_window = 260
    ...
    sell_overbought_rsi = 88.0
    take_profit_pnl_pct = 7.0
    ...
    def next(self) -> None:
        """Called by backtesting.py on each daily bar."""
    ...
    def _current_shares(self) -> int:
        """Return current integer XRP token quantity held."""
    ...
    def _price_with_slippage(self, price: float, is_buy: bool) -> float:
        ...
        Args:
            ...
            is_buy: True for buy fills (price increases), False for sell.
    ...
    def _build_packet(self) -> Dict[str, Any] | None:
        """Build the market+portfolio packet for action determination.

        Returns:
            Packet dict, or None if insufficient history.
        """
        ...
        # Soft 0.90x damper after persistent adverse regime; entries still allowed.
        ...
        # At XRP $2-3: risk_amount=$2k / (ATR $0.10 * 2) = 10,000 tokens (~$25k notional).
        # max_position_size_pct cap brings this down to the allowed position limit.
        ...
        packet = {
            ...
            "market_data": {
                "price": round(current_price, 4),  # 4 dp for fractional XRP pricing
                "history": [round(v, 4) for v in closes[-60:]],
                ...
                "stop_loss_suggestion": round(current_price - (atr_14 * 2.0), 4),
                "take_profit_suggestion": round(current_price + (atr_14 * 3.0), 4),
    ...
    def _determine_action(self, packet: Dict[str, Any]) -> Tuple[str, str]:
        """Determine BUY / SELL / HOLD for the current bar.
    ...
    def _execute_action(self, action: str, packet: Dict[str, Any]) -> None:
        """Route the action to the appropriate execution method, with ATR and drawdown guards.
    ...
    def _execute_buy(self, packet: Dict[str, Any]) -> None:
        """Size and submit a buy order, then update cost basis.
        ...
        # Enforce 1-token minimum when base signal was positive and capacity allows.
    ...
    def _execute_sell(self, packet: Dict[str, Any]) -> None:
        """Size and submit a sell order using crypto-appropriate partial sell bands.
        ...
        # Crypto-adjusted partial sell bands. XRP commonly moves 3-8% per day, so
        # the MSFT 8% full-exit floor would trigger as routine noise. Bands widened:
        #   <15%  → full exit  (small gain or loss, close cleanly)
        #   15-40% → 30% partial
        #   40-80% → 40% partial
        #   80%+   → 60% partial (let a XRP mega-rally run)
        if unrealized_pnl_pct < 15.0:
            sell_pct = 1.0
        elif unrealized_pnl_pct < 40.0:
            sell_pct = 0.30
        elif unrealized_pnl_pct < 80.0:
            sell_pct = 0.40
        else:
            sell_pct = 0.60
