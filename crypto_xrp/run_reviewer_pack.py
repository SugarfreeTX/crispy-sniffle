...
CRYPTO_DIR = ROOT / "crypto_xrp"
BACKTEST_PATH = CRYPTO_DIR / "backtest.py"
BACKTEST_OUTPUTS = CRYPTO_DIR / "backtest_outputs"
DATA_PATH = CRYPTO_DIR / "data" / "xrp_daily.csv"
NOTEBOOK_PATH = CRYPTO_DIR / "reviewer_pack_analysis.ipynb"

# NOTE: portfolio["shares"] represents integer XRP token count, not equity shares.
# This is inherited from shared/types.py PortfolioDict — same field name, different semantic.


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run full XRP reviewer-pack pipeline and generate a handoff zip."
    )
...
def _scenario_commands(args: argparse.Namespace) -> List[List[str]]:
    ...
    return [
        # Refresh cached XRP-USD data from Yahoo Finance, then run baseline.
        [py, bt, "--refresh-data", *primary],
        # Parameter sweep with crypto-calibrated ATR and RSI ranges.
        [
            py,
            bt,
            *primary,
        ...
        # Stress: elevated slippage (20 bps) to model thin crypto book conditions.
        [py, bt, *primary, "--commission-bps", "5", "--slippage-bps", "20"],
        # Ablation: disable loss-streak cap.
        [py, bt, *primary, "--max-consecutive-losses", "999"],
        # Ablation: disable min-ATR filter (0.0 = trade regardless of volatility level).
        [py, bt, *primary, "--min-atr", "0.0"],
        # Ablation: disable overbought RSI sell gate.
        [py, bt, *primary, "--sell-overbought-rsi", "100"],
        # Ablation: disable take-profit exits.
        [py, bt, *primary, "--take-profit-pnl-pct", "100", "--take-profit-rsi", "100"],
        # Walk-forward windows — same calendar splits as the equity pack for comparability.
...
def _build_pack(args: argparse.Namespace) -> Dict[str, Path]:
    ...
    pack_dir = CRYPTO_DIR / f"reviewer_pack_{pack_tag}"
    zip_path = CRYPTO_DIR / f"reviewer_pack_{pack_tag}.zip"
    marker_path = CRYPTO_DIR / f".reviewer_pack_marker_{pack_tag}"
    current_pack_tag_path = CRYPTO_DIR / ".current_pack_tag"
    latest_pack_path = CRYPTO_DIR / ".latest_reviewer_pack.txt"
    ...
    # XRP-USD filenames use underscores (xrp_usd) since backtest.py replaces '-' with '_'.
    metrics_df = _read_metrics_rows((pack_dir / "backtest_outputs").glob("metrics_xrp_usd_*.json"))
    ...
    sweep_best_files = sorted((pack_dir / "backtest_outputs").glob("sweep_best_xrp_usd_*.json"))
    ...
    counts = {
        "metrics": len(list((pack_dir / "backtest_outputs").glob("metrics_xrp_usd_*.json"))),
        "equity": len(list((pack_dir / "backtest_outputs").glob("equity_xrp_usd_*.csv"))),
        "trades": len(list((pack_dir / "backtest_outputs").glob("trades_xrp_usd_*.csv"))),
        "stats": len(list((pack_dir / "backtest_outputs").glob("stats_xrp_usd_*.txt"))),
        "sweep": len(list((pack_dir / "backtest_outputs").glob("sweep_*_xrp_usd_*.csv")))
        + len(list((pack_dir / "backtest_outputs").glob("sweep_best_xrp_usd_*.json"))),
    }
